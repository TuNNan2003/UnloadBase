...
RC RecordPageHandler::update_record(const RID *rid, const Record &record)
{
  ...
  Bitmap bitmap(bitmap_, page_header_->record_capacity);
  if(bitmap.get_bit(rid->slot_num)) {
    char * origin = get_record_data(rid->slot_num);
    memcpy(origin, record.data(), record.len());
...
RC RecordFileHandler::update_record(const RID *rid, const Record &record)
{
  RC rc = RC::SUCCESS;
  RecordPageHandler page_handler;
  ...
  rc = page_handler.update_record(rid, record);
  return rc;
}
...
RC RecordFileHandler::get_record(RecordPageHandler &page_handler, const RID *rid, bool readonly, Record *rec)
{
  ...
  if (ret != RC::RECORD_OPENNED && OB_FAIL(ret)) {
