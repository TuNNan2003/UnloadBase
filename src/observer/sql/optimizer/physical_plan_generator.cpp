...
RC PhysicalPlanGenerator::create_plan(PredicateLogicalOperator &pred_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  for (int i = 0; i < children_opers.size(); i++)
  {
    ...
    case LogicalOperatorType::PROJECTION:
    {
      ...
      ProjectLogicalOperator *proj_child_oper = static_cast<ProjectLogicalOperator *>(children_opers.at(i).get());
    ...
    case LogicalOperatorType::TABLE_GET:
    {
      ...
      TableGetLogicalOperator *proj_child_oper = static_cast<TableGetLogicalOperator *>(children_opers.at(i).get());
...
RC PhysicalPlanGenerator::create_plan(ProjectLogicalOperator &project_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  const std::vector<std::unique_ptr<Expression>> &exprs = project_oper.selectExprs();

  ProjectPhysicalOperator *project_operator = new ProjectPhysicalOperator(exprs);
...
RC PhysicalPlanGenerator::create_plan(InsertLogicalOperator &insert_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  const vector<vector<Value>> &values = insert_oper.values();
...
RC createTableScanWithPredicate(PredicateLogicalOperator &pred_oper, unique_ptr<PhysicalOperator> &oper){
  vector<unique_ptr<LogicalOperator>> &children_opers = pred_oper.children();
  ...
  unique_ptr<PhysicalOperator> child_phy_oper;
  RC rc=createTableScanWithTableGet(static_cast<TableGetLogicalOperator &>(child_oper), child_phy_oper);
  if (rc != RC::SUCCESS) {
    LOG_WARN("failed to create child operator of predicate operator. rc=%s", strrc(rc));
    return rc;
  }
...
RC PhysicalPlanGenerator::create_plan(DeleteLogicalOperator &delete_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  if (!child_opers.empty())
  {
    ...
    if(child_oper->type()==LogicalOperatorType::TABLE_GET){
      ...
      rc=createTableScanWithPredicate(static_cast<PredicateLogicalOperator &>(*child_oper), child_physical_oper);
    }
    else{
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(update_oper.GetTable(), update_oper.SetNode()));
