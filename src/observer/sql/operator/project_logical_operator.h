...
{
public:
  ProjectLogicalOperator(std::vector<std::unique_ptr<Expression>> &exprs);
  ...
  const std::vector<std::unique_ptr<Expression>> &selectExprs() const
  {
    return selectExprs_;
  }
...
private:
  ...
  //! 或者是执行某个函数。所以这里是表达式Expression。
  std::vector<std::unique_ptr<Expression>>  &selectExprs_;
};
